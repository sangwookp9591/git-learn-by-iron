...
export function explainFailure(failure: Failure): string {
  ...
  switch (aliases[failure.assertion] ?? failure.assertion) {
    ...
    case 'head.message.matches': return `최근 커밋 메시지가 요청한 형식과 달라요. 현재 메시지: ${String(failure.actual)}. 이 실습에서는 처음부터 다시 시작해 메시지를 고칠 수 있어요.`;
    ...
    default: return `아직 확인할 수 없는 조건이 있어요: ${failure.assertion}. 다른 레슨을 선택하거나 처음부터 다시 시작하세요.`;
  }
}
